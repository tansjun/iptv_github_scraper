...
├── .github/workflows/
│   ├── run-iptv.yml            # 抓取工作流（手机触发入口）
│   └── static.yml              # GitHub Pages 部署（live 文件发布）
└── README.md
```
...
4. （可选）启用 GitHub Pages：仓库 Settings → Pages → Source 选 `Deploy from a branch` → 分支选 `gh-pages` → Save。启用后 live 文件可通过 Pages 链接访问（见下）。
...
- Pages 链接（启用后）：`https://<用户名>.github.io/<仓库名>/hn-live.txt`
